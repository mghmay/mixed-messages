...
//Below are the initial global variables in an object. The inital values are important becaude the form a part of the prompt for use input. This object is never changed.
let variableObj = {
...
//Below are the madlib stories. they containt template literals however you will notice that they are not surrounded by backticks. A function later on will do this for us but it is important for now that we don't incluse the upticks because this would could it to run on declaration.
const madLibs = [
...
// this imports prompt-synx which allows us to ask for used input on the command line.
...
//The below function gets an array of all of the variables within the randomly generated madlib.
function getVariableArr(madLib) {
  let myRegex = /(?<=\${this.)\w+/gi;
  let returnVars = madLib.match(myRegex);
  return returnVars.filter((item, index) => returnVars.indexOf(item) === index);
}
// The below prompts for user input to set the variables for our generated madlib. It returns an array of variable - input pairs.
